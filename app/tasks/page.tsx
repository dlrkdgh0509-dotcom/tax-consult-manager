export const dynamic = "force-dynamic";
import AppLayout from "@/components/AppLayout";
import { supabase } from "@/lib/supabase";
import Link from "next/link";

type TaskWithCustomer = {
    id: string;
    customer_id: string;
    title: string;
    due_date: string | null;
    status: string | null;
    memo: string | null;
    created_at: string;
    customers: {
        id: string;
        name: string;
        phone: string | null;
    } | null;
};

export default async function TasksPage() {
    const { data: tasks, error } = await supabase
        .from("tasks")
        .select(`
            *,
            customers (
                id,
                name,
                phone
            )
        `)
        .order("due_date", { ascending: true });

    const taskList = (tasks ?? []) as TaskWithCustomer[];

    return (
        <AppLayout>
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h2 className="text-3xl font-bold mb-2 text-gray-900">
                        할일관리
                    </h2>

                    <p className="text-gray-500">
                        자료 요청, 신고기한, 다음 연락일을 관리합니다.
                    </p>
                </div>

                <Link
                    href="/tasks/new"
                    className="px-5 py-3 bg-green-600 text-white rounded-xl font-semibold"
                >
                    할 일 등록
                </Link>
            </div>

            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                {error && (
                    <div className="p-4 mb-4 rounded-xl bg-red-50 text-red-600">
                        할 일 목록을 불러오지 못했습니다.
                    </div>
                )}

                {!error && taskList.length === 0 && (
                    <div className="p-10 text-center text-gray-500">
                        아직 등록된 할 일이 없습니다.
                    </div>
                )}

                <div className="space-y-3">
                    {taskList.map((task) => (
                        <div
                            key={task.id}
                            className="border border-gray-100 rounded-xl p-5 flex items-center justify-between"
                        >
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    <strong className="text-lg text-gray-900">
                                        {task.title}
                                    </strong>

                                    <span
                                        className={
                                            task.status === "완료"
                                                ? "text-sm px-3 py-1 rounded-full bg-gray-100 text-gray-600"
                                                : "text-sm px-3 py-1 rounded-full bg-red-50 text-red-600"
                                        }
                                    >
                                        {task.status || "미완료"}
                                    </span>
                                </div>

                                <p className="text-sm text-gray-500">
                                    {task.customers?.name || "고객 없음"} · 마감일:{" "}
                                    {task.due_date || "미지정"}
                                </p>

                                {task.memo && (
                                    <p className="text-sm text-gray-700 mt-2 whitespace-pre-wrap">
                                        {task.memo}
                                    </p>
                                )}
                            </div>

                            <div className="flex gap-2">
                                {task.customers?.id && (
                                    <Link
                                        href={`/customers/${task.customers.id}`}
                                        className="px-4 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 text-gray-700"
                                    >
                                        고객 보기
                                    </Link>
                                )}

                                <Link
                                    href={`/tasks/${task.id}/edit`}
                                    className="px-4 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 text-gray-700"
                                >
                                    수정
                                </Link>
                            </div>
                        </div>
                    ))}
                </div>
            </div>
        </AppLayout>
    );
}